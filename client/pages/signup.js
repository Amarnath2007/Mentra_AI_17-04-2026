...
import { Sparkles, User, Mail, Lock, Eye, EyeOff, AlertCircle, GraduationCap, Users, ChevronRight, ChevronLeft, Heart } from 'lucide-react';

const INTEREST_OPTIONS = [
  'Programming', 'Artificial Intelligence', 'Data Science', 'UI/UX Design', 
  'Marketing', 'Entrepreneurship', 'Blockchain', 'Cybersecurity', 
  'FinTech', 'Cloud Computing', 'Game Development', 'Digital Arts', 
  'Content Creation', 'Management', 'Psychology', 'E-commerce'
];
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-slate-50">
      ...
      <div className="relative z-10 w-full max-w-md animate-slide-up">
        ...
        <div className="glass rounded-2xl p-8">
          {error && (
            <div className="flex items-center gap-2 p-3 rounded-lg mb-4 text-sm" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#ef4444' }}>
              <AlertCircle size={15} />
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
               ...
               <div className="grid grid-cols-2 gap-3">
                 {[
                   ...
                   <button key={value} type="button" onClick={() => setForm(p => ({ ...p, role: value }))}
                     ...
                     style={{
                       background: form.role === value ? `${color}15` : 'white',
                       border: `1px solid ${form.role === value ? color + '50' : '#e2e8f0'}`,
                     }}>
             ...
             <button type="submit" disabled={loading} className="btn-primary w-full justify-center mt-2 group">
                {loading ? (
