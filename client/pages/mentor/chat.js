...
export default function MentorChat() {
  ...
  return (
    <MentorLayout title="Messages">
      <div className="h-[calc(100vh-160px)] flex bg-[#F9FAFB] dark:bg-slate-800/60 rounded-3xl border border-[#E5E7EB] dark:border-slate-700 overflow-hidden shadow-sm animate-fade-in">
        
        {/* Sidebar: Contacts */}
        <div className="w-80 border-r border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-900 flex flex-col">
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
               <h3 className="text-xs font-800 text-[#111827] dark:text-white uppercase tracking-widest">Active Chats</h3>
            ...
            <div className="relative group">
              ...
              <input 
                ...
                className="w-full bg-[#F9FAFB] dark:bg-slate-800/60 border border-[#E5E7EB] dark:border-slate-700 rounded-xl py-2.5 pl-10 pr-4 text-xs font-500 focus:outline-none focus:border-[#0A66C2] transition-colors"
                value={search}
          ...
          <div className="flex-1 overflow-y-auto translate-z-0 custom-scrollbar">
            {loading ? (
               [1,2,3].map(i => <div key={i} className="h-16 bg-slate-50 dark:bg-slate-800 m-4 rounded-xl animate-pulse" />)
            ) : filteredContacts.map((contact) => (
              <button 
                key={contact.id}
                onClick={() => setSelectedContact(contact)}
                className={`w-full flex items-center gap-4 px-6 py-5 border-b border-transparent transition-all relative ${selectedContact?.id === contact.id ? 'bg-[#0A66C2]/5' : 'hover:bg-[#F9FAFB] dark:bg-slate-800/60'}`}
              >
                {selectedContact?.id === contact.id && <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#0A66C2]" />}
                <div className="relative shrink-0">
                  <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[#0A66C2] font-700 text-sm border border-[#E5E7EB] dark:border-slate-700">
                    {contact.name[0]}
                  </div>
                </div>
                <div className="flex-1 min-w-0 text-left">
                  <p className={`text-sm font-700 truncate ${selectedContact?.id === contact.id ? 'text-[#0A66C2]' : 'text-[#111827] dark:text-white'}`}>{contact.name}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 truncate font-500">{contact.experience_level || 'Student'}</p>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-slate-900">
          {selectedContact ? (
            <>
              {/* Chat Header */}
              <div className="h-20 border-b border-[#E5E7EB] dark:border-slate-700 flex items-center justify-between px-8 bg-white dark:bg-slate-900/80 backdrop-blur-md sticky top-0 z-10">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-[#F9FAFB] dark:bg-slate-800/60 border border-[#E5E7EB] dark:border-slate-700 flex items-center justify-center text-[#0A66C2] font-700">
                    {selectedContact.name[0]}
                  </div>
                  <div>
                    <p className="text-sm font-700 text-[#111827] dark:text-white">{selectedContact.name}</p>
                ...
                <button className="p-2 text-slate-400 hover:text-[#0A66C2] hover:bg-blue-50 dark:bg-blue-900/20 rounded-xl transition-all">
              ...
              <div 
                ref={scrollRef}
                className="flex-1 overflow-y-auto p-8 space-y-6 bg-white dark:bg-slate-900 custom-scrollbar"
              ...
              >
                    ...
                    <div key={msg.id || i} className={`flex ${isMentor ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] group`}>
                        <div className={`px-4 py-3 rounded-2xl text-sm font-500 leading-relaxed shadow-sm ${
                          ...
                          : 'bg-[#F3F4F6] dark:bg-slate-800 text-[#111827] dark:text-white rounded-bl-none border border-slate-100 dark:border-slate-700'
                        }`}>
                          {msg.content}
                        </div>
                ...
                {typing && (
                  <div className="flex justify-start animate-fade-in">
                    <div className="px-4 py-3 rounded-2xl bg-[#F3F4F6] dark:bg-slate-800 text-slate-400 flex gap-1">
              ...
              {/* Input Area */}
              <div className="p-6 border-t border-[#E5E7EB] dark:border-slate-700 bg-white dark:bg-slate-900">
                <form onSubmit={handleSendMessage} className="flex items-end gap-4 bg-[#F9FAFB] dark:bg-slate-800/60 border border-[#E5E7EB] dark:border-slate-700 rounded-2xl p-2 px-4 focus-within:border-[#0A66C2] transition-colors">
                  ...
                  <textarea 
                    rows="1"
                    className="flex-1 bg-transparent py-2.5 px-2 text-sm text-[#111827] dark:text-white font-500 focus:outline-none resize-none placeholder:text-slate-400"
                  ...
                  <button 
                    ...
                    className={`p-2.5 rounded-xl transition-all ${inputText.trim() ? 'bg-[#0A66C2] text-white shadow-lg shadow-blue-200 dark:shadow-blue-900/30' : 'bg-[#E5E7EB] text-white cursor-default'}`}
                  >
                    <Send size={18} />
                  </button>
                </form>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-300 space-y-4">
              <div className="w-20 h-20 rounded-3xl bg-[#F9FAFB] dark:bg-slate-800/60 flex items-center justify-center">
