          ...
          <div className="flex bg-[#F3F4F6] p-1 rounded-xl w-fit border border-slate-200">
            {['all', 'submitted', 'in-progress', 'reviewed'].map(s => (
              <button 
                ...
                className={`px-4 py-1.5 text-xs font-700 rounded-lg transition-all capitalize ${statusFilter === s ? 'bg-white text-[#0A66C2] shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >
                {s.replace('-', ' ')}
              </button>
            ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            {loading ? (
              ...
              <div key={task.id} className="bg-white p-6 rounded-3xl border border-[#E5E7EB] hover:border-[#0A66C2]/30 transition-all flex flex-col md:flex-row md:items-center justify-between gap-6 group shadow-sm">
                <div className="flex items-start gap-4 flex-1">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 border ${
                    task.status === 'submitted' ? 'bg-amber-50 border-amber-100 text-amber-500' :
                    task.status === 'reviewed' ? 'bg-emerald-50 border-emerald-100 text-emerald-500' :
                    'bg-blue-50 border-blue-100 text-[#0A66C2]'
                  }`}>
                    <FileText size={20} />
                  </div>
                  <div>
                    <h4 className="text-base font-700 text-[#111827] mb-1 group-hover:text-[#0A66C2] transition-colors">{task.title}</h4>
                    <div className="flex flex-wrap items-center gap-4 text-xs font-600 text-slate-500 mt-2">
                       <span className="flex items-center gap-1.5 text-[#111827]"><Users size={14} className="text-slate-400" /> {task.student}</span>
                ...
                <div className="flex items-center gap-3">
                  {task.status === 'submitted' ? (
                    ...
                    <button className="px-5 py-2.5 bg-white text-slate-600 border border-[#E5E7EB] rounded-xl text-xs font-700 hover:border-[#0A66C2] hover:text-[#0A66C2] transition-all">
                       View Details
                    </button>
                  )}
            ...
            {filteredTasks.length === 0 && (
              <div className="py-20 text-center bg-[#F9FAFB] rounded-3xl border-2 border-dashed border-[#E5E7EB]">
          ...
          <div className="space-y-8">
            <div className="bg-[#111827] p-8 rounded-3xl text-white relative overflow-hidden">
               ...
               <div className="bg-white/5 backdrop-blur-md rounded-2xl p-5 border border-white/10 italic">
               ...
               <button className="mt-8 w-full py-3 bg-[#00C4B4] text-[#111827] rounded-2xl text-xs font-800 shadow-xl shadow-teal-900/40 hover:scale-[1.02] transition-transform">
                 Scan Active Submissions
               </button>
            </div>

            <div className="bg-white p-8 rounded-3xl border border-[#E5E7EB] shadow-sm">
               <h4 className="text-xs font-800 text-[#111827] uppercase tracking-widest mb-6 flex items-center gap-2">
               ...
               <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3 p-3 rounded-2xl bg-[#F9FAFB] border border-[#E5E7EB] hover:border-[#0A66C2]/30 transition-all cursor-pointer">
                       <div className="w-8 h-8 rounded-lg bg-white border border-[#E5E7EB] flex items-center justify-center text-[#0A66C2] font-800 text-[10px]">
                         {sub.name[0]}
                       </div>
                       <div className="flex-1 min-w-0">
                          <p className="text-[11px] font-700 text-[#111827] truncate">{sub.name}</p>
